# Purpose: Delineate watersheds based on DEM (digital elevation map) - FOR EXISTING ES SITES
...
es_coords_near <- es_coords_novelt_polis %>%
  filter( !(site_name %in% c('BALDIA; MUHAMMAD KHAN COLONY',
                          'COMPOSITE KORANGI NALLA',
                          'BAKHTAWAR VILLAGE',
                          'HIJRAT COLONY PIDC NALA')))
es_coords_far <- es_coords_novelt_polis %>%
  filter(site_name %in% c('BALDIA; MUHAMMAD KHAN COLONY',
                             'COMPOSITE KORANGI NALLA',
                             'BAKHTAWAR VILLAGE',
                             'HIJRAT COLONY PIDC NALA'))
...
stream_points <- st_as_sf(streams) %>% 
  st_cast('POINT')
# Find nearest stream point for each ES site
nearest_index <- st_nearest_feature(es_coords_near, 
                                    stream_points)
nearest_pts = stream_points[nearest_index,]
...
for (i in seq_len(nrow(nearest_pts))) {
  # extract respective point
  tmp_coords <- nearest_pts[i,] 
  message(paste0(i, ' out of ', nrow(nearest_pts)))
  # calculate drainage area
  execGRASS("r.water.outlet", 
            flags="overwrite", 
            parameters=list(input = "fdir",
                            output = paste0("watershed_", i),
                            coordinates = st_coordinates(nearest_pts)[i,]))
  ...
  execGRASS('v.out.ogr', flags=c('overwrite'),
            parameters=list(input=paste0("basin_", i),
                            output=paste0("results/es_watersheds/watershed_",
                                          i, 
                                          ".shp"),
...
files <- list.files("results/es_watersheds", pattern = "watershed_[0-9]{,3}.shp")
shp_basins <- st_read(file.path("results/es_watersheds", files[1])) %>%
  mutate(file = files[1])
for(i in 2:length(files)){
  shp_tmp <- st_read(file.path("results/es_watersheds", files[i])) %>%
    mutate(file = files[i])
...
test <- pop
test@data@values
test@data@values[basin_pop_raster_index[[7]]] <- 5000
tm_shape(pop) +
  tm_raster() +
  tm_shape(shp_basins[7,]) +
  tm_polygons(col = '#FDC68C', alpha = .05) 
tm_shape(test) +
  tm_raster() +
  tm_shape(shp_basins[7,]) +
...
save(shp_basins,
     ...
     es_coords_near,
     es_coords_far,
     file = 'results/es_watersheds_in_karachi.Rdata')

load('results/es_watersheds_in_karachi.Rdata')
