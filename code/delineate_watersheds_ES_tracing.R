# Purpose: Delineate watersheds based on DEM (digital elevation map) - FOR POTENTIAL ES TRACING POINTS
...
for (i in seq_len(nrow(tracing_pts))) {
  # extract respective point
  tmp_coords <- tracing_pts[i,] 
  message(paste0(i, ' out of ', nrow(tracing_pts)))
  # calculate drainage area
  execGRASS("r.water.outlet", 
            flags="overwrite", 
            parameters=list(input = "fdir",
                            output = paste0("watershed_", i),
                            coordinates = st_coordinates(tracing_pts)[i,]))
  ...
  execGRASS('v.out.ogr', flags=c('overwrite'),
            parameters=list(input=paste0("basin_", i),
                            output=paste0("results/tracing_watersheds/watershed_",
                                          i, 
                                          ".shp"),
...
files <- list.files("results/tracing_watersheds", pattern = "watershed_[0-9]{,3}.shp")
tracing_basins <- st_read(file.path("results/tracing_watersheds", files[1])) %>%
  mutate(file = files[1])
for(i in 2:length(files)){
  shp_tmp <- st_read(file.path("results/tracing_watersheds", files[i])) %>%
    mutate(file = files[i])
  tracing_basins <- rbind(tracing_basins, shp_tmp)
}
tracing_basins <- tracing_basins %>%
  mutate(id = file,
         id = as.numeric(gsub('watershed_|\\.shp', '', id)))
table(table(tracing_basins$id))  # Some basins have 'danglers' that get counted as another feature
plot(tracing_basins %>% filter(id ==5))
...
# tmp <- st_intersects(tracing_basins, shp_khi)
# tracing_basins$intersects_khi <- sapply(tmp, length) > 0
# basin_in_khi <- tracing_basins %>% 
#   group_by(id) %>%
#   summarize(in_khi = sum(intersects_khi) > 0) %>%
#   filter(in_khi == T) %>%
#   pull(id)
# streams <- st_as_sf(streams) %>% filter(stream %in% basin_in_khi)
# nearest_pts <- nearest_pts %>% filter(id %in% basin_in_khi)
# tracing_basins <- tracing_basins %>% filter(id %in% basin_in_khi)
...
tmp <- as(tracing_basins, "Spatial")
index <- extract(pop, tmp, cellnumbers = T)
tracing_basin_pop_raster_index <- lapply(index, function(x) x[,'cell'])
...
# library(nngeo)
# t2 <- system.time(raster_extract(dem, tracing_basins[1,], fun = sum, na.rm = TRUE))

test <- pop
test@data@values
test@data@values[tracing_basin_pop_raster_index[[7]]] <- 5000
tm_shape(pop) +
  tm_raster() +
  tm_shape(tracing_basins[7,]) +
  tm_polygons(col = '#FDC68C', alpha = .05) 
tm_shape(test) +
  tm_raster() +
  tm_shape(tracing_basins[7,]) +
...
save(tracing_basins,
     ...
     # es_coords_near,
     # es_coords_far,
     file = 'results/tracing_watersheds_in_karachi.Rdata')
