...
# stream_coords
# nearest_pts
es_shp_basins <- shp_basins
es_nearest_pts <- nearest_pts
...
es_shp_basins$pop_sum <- pop_sum
es_nearest_pts$pop_sum <- pop_sum
...
save(es_shp_basins,
     ...
     es_coords_near,
     es_coords_far,
     file = 'results/es_watersheds_in_karachi_pop.Rdata')
...
p <- tm_shape(shp_khi) +
  ...
  # Buffer around far points
  tm_shape(es_coords_far) +
  tm_dots(col = '#FDC68C', size = 1, alpha = 0.5) +
  tm_shape(es_coords_far) +
  tm_dots(col = '#E55383', size = .15) +
...
#' # Streams & sample points + WATERSHEDS + ES sites
#' p <- tm_shape(shp_khi) +
#'   tm_polygons(col = '#FDC68C', alpha = 0, border.col = 'black') +
#'   tm_shape(shp_basins) +
#'   tm_polygons(col = '#FDC68C', alpha = 0.5,  border.col = 'grey80') +
#'   tm_shape(streams) +
#'   tm_lines(col = '#30A9DE', lwd = 2) +
